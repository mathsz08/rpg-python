...
class UI:
    def __init__(self):
        ...
        for magic in magic_data.values():
            magic_im = pygame.image.load(magic['graphic']).convert_alpha()
            self.magic_graphics.append(magic_im)
    ...
    def selection_box(self,left,top, has_switched):
        bg_rect = pygame.Rect(left,top,ITEN_BOX_SIZE, ITEN_BOX_SIZE)
        pygame.draw.rect(self.display_surface, UI_BG_COLOR,bg_rect)
        if has_switched:
    ...
    def magic_overlay(self,magic_index, has_switched):
        bg_rect = self.selection_box(80,585,has_switched)
